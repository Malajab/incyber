...
- FALSE POSITIVE (FP) ويعني أن نظام الرصد للتهديدات قام بتصنيف حدث event أو الاتصال غير ضار بأنه ضار، وهذا عادة ما يسبب إهدار وقت المحلل في تحليل أحداث أو تنبيهات أمنية غير صحيحة.
- FALSE NEGATIVE (FN) ويعني أن نظام الرصد للتهديدات فشل في رصد حدث event أو اتصال ضار، وهذا أكثر خطورة حيث إنه يجب أن يتم تحديث آليات الرصد بشكل مستمر لكي ترصد التهديدات السيبرانية.
- TRUE POSITIVE (TP) ويعني أن نظام رصد التهديدات نجح في تصنيف الحدث event أو الاتصال على أنه ضار وقام باتخاذ الإجراء.
- TRUE NEGATIVE (TN) ويعني أن النظام رصد تهديد ويدرك أنه تهديد ولكن لا يصدر تنبيه حيث إن التهديد لا يجب اتخاذ إجراء ضده.
...
- physical layer تهتم بالوسائط المادية والإشارات
- data link layer تنقل البيانات بين الأجهزة في نفس الشبكة وتتعامل مع الأخطاء
- network layer تتحكم في توجيه البيانات بين الشبكات
- transport layer تضمن نقل البيانات بشكل موثوق
- session layer تدير وتنهي الاتصالات (الجلسات) بين التطبيقات
- presentation layer تنسيق وترميز البيانات
- application layer تتعامل مع التطبيقات التي تستخدم الشبكة
